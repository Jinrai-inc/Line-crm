...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, eventDate, startTime, endTime, location, locationUrl, capacity, status: seminarStatus, registrationDeadline, tag_ids, paymentUrl, zoomUrl, price, postPaymentUrl, zoomNote, postPaymentMessage } = body
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        price: price || null,
        post_payment_url: postPaymentUrl || null,
        zoom_note: zoomNote || null,
        post_payment_message: postPaymentMessage || null,
      })
      .select()
      .single()
