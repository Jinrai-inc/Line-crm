...
const manualSections: ManualSection[] = [
  {
    ...
    items: [
      {
        question: "【NEW】セミナー参加者の決済状況を一覧で確認",
        answer:
          "セミナー詳細ページの参加者一覧に、Stripe決済の状況がリアルタイムで表示されるようになりました。\n\n■ 表示される内容\n・申込数 / 決済完了 / 未決済 のサマリーカード\n・参加者ごとの決済状況バッジ（決済完了／未決済／返金済／決済失敗）\n\n■ データの集計ルール\n・キャンセル済みの申込はサマリーから除外\n・Stripe Webhookで決済完了が通知された時点で自動的に「決済完了」に更新\n・無料セミナーや決済リンク未設定のセミナーは全員「未決済」と表示されます（決済レコードが作成されないため）\n\n確認方法：セミナー管理 → 対象セミナーを選択 → 参加者一覧",
      },
      {
        question: "【NEW】セミナー申込時のタグ自動付与",
        answer:
          "セミナーに事前にタグを設定しておくと、LINEからそのセミナーに申し込んだ友だちへ自動でタグが付与されます。\n\n■ 活用例\n・「4月セミナー参加」タグを自動付与\n・特定のセミナー経由で来た友だちを自動でセグメント化\n・申込後のフォローアップ配信のターゲット指定に利用\n\n設定方法：セミナー作成・編集画面 → 「タグ」で付与したいタグを選択",
      },
      {
        question: "【NEW】セミナーごとに決済後URLを設定",
        answer:
          "決済完了後に送信するURL（予約ページ等）と案内文をセミナーごとにカスタマイズできるようになりました。TimeRex等の個別予約フォームへの導線として活用できます。\n\n■ 設定項目\n・決済後に送るURL：お支払い確認メッセージの後に送信されるリンク\n・決済後URLの案内文：ボタンの上に表示される説明文（改行対応）\n\n設定方法：セミナー作成・編集画面 → 決済・参加リンク設定 → 「決済後に送るURL」「決済後URLの案内文」",
      },
      {
        question: "【NEW】日付未定セミナーの作成",
        answer:
          "開催日が確定していないセミナーを「日付未定」として作成できます。日程調整中の事前告知や、参加希望者を先に集めたい場合に便利です。\n\n■ 使い方\n・セミナー作成／編集画面で「日付未定」チェックボックスをONにする\n・LINE側では「日付調整中」と表示されます\n・日程が決まったら同じ画面から日付を追記するだけでOK",
      },
      {
        question: "【NEW】残りわずか／満員の自動表示",
        answer:
          "LINE側のセミナー案内で、残席数そのものは非表示にしつつ、状況に応じて「残りわずか」「満員」のラベルが自動表示されるようになりました。\n\n■ 表示ルール\n・残り5席以下：「残りわずか」と赤字で表示\n・定員到達：「満員御礼」と表示し、申込ボタンを非活性化\n・それ以外：ラベルなし（残席数は表示しません）\n\n過度な席数開示を避けつつ、申込の後押しにつなげられます。",
      },
      {
